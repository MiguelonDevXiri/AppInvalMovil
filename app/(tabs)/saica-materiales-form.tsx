...
export default function SaicaMaterialesFormScreen() {
  ...
  useEffect(() => {
    const loadNotes = async () => {
      ...
      if (paramNotes) {
        ...
        try {
          const savedDraftNotes = await AsyncStorage.getItem('saica_draft_notes');
          if (savedDraftNotes) {
  ...
  const handleContinue = async () => {
    ...
    try {
      await AsyncStorage.setItem('saica_draft_notes', notes);
    } catch (error) {
      console.error('No se pudo guardar el borrador de notas:', error);
    }

    router.push({
      pathname: '/(tabs)/saica-report-view' as any,
      params: {
        ...params,
        notes,
