...
import {
  ...
  paramsToInspection,
  saveSaicaInspection,
  type SaicaInspection,
} from '../../utils/saicaInspectionStorage';
import { shareSaicaPDFReport } from '../../utils/saicaReportGenerator';

export default function SaicaReportViewScreen() {
  const params = useLocalSearchParams();
  const [isGenerating, setIsGenerating] = useState(false);
  const [isSharing, setIsSharing] = useState(false);
  const [progressText, setProgressText] = useState('');
  const [progressPercent, setProgressPercent] = useState(0);
  const [inspection, setInspection] = useState<SaicaInspection | null>(null);
  ...
  const loadAndSave = async () => {
    try {
      ...
      const draftNotes = await AsyncStorage.getItem('saica_draft_notes');
  ...
  const autoGeneratePDF = async (insp: SaicaInspection) => {
    try {
      setIsGenerating(true);
      setProgressText('Guardando inspección...');
      setProgressPercent(5);

      const saved = await saveSaicaInspection(insp);
      try {
        await AsyncStorage.removeItem('saica_draft_notes');
      } catch (error) {
      ...
      const success = await shareSaicaPDFReport(saved, (percent, text) => {
  ...
  const handleSharePDF = async () => {
    ...
    try {
      ...
      await shareSaicaPDFReport(inspection, (percent, text) => {
        setProgressPercent(percent);
        setProgressText(text);
      }, false); // compartir
    } catch (error) {
  ...
  const handleEdit = () => {
    if (!inspection) return;
    router.push({ pathname: '/saica-machine-form' as any, params: { ...inspectionToParams(inspection), isEditing: 'true' } });
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        {inspection.interventions && inspection.interventions.length > 0 && (
          <Card style={styles.card}>
            <Card.Content>
              <Title style={styles.sectionTitle}>🔧 Intervenciones necesarias ({inspection.interventions.length})</Title>
              <Divider style={styles.dividerLine} />
              {inspection.interventions.map((intervention, idx) => (
                <View key={intervention.id || idx} style={styles.interventionSection}>
                  <Text style={styles.interventionLabel}>Intervención {idx + 1}</Text>
                  <Paragraph>{intervention.description}</Paragraph>
                  {intervention.photos && intervention.photos.length > 0 && (
                    <LazyPhotoGrid title={`Fotos intervención ${idx + 1}`} photos={intervention.photos} labelPrefix="I" accentColor="#7c3aed" />
                  )}
                  {idx < inspection.interventions.length - 1 && <Divider style={{ marginVertical: SPACING.sm }} />}
                </View>
              ))}
            </Card.Content>
          </Card>
...
const styles = StyleSheet.create({
  ...
  interventionSection: { marginBottom: SPACING.sm },
  interventionLabel: { fontWeight: TYPOGRAPHY.weights.bold as any, color: '#7c3aed', marginBottom: SPACING.xs },
