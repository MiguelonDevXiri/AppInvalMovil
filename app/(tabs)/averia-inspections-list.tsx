...
interface CardProps {
  ...
  onPress: (i: AveriaInspection) => void;
  onMenuPress?: (i: AveriaInspection, e?: any) => void;
  onLongPress?: (i: AveriaInspection) => void;
  isSelectionMode?: boolean;
  isSelected?: boolean;
}

const AveriaCard = ({ inspection, onPress, onMenuPress, onLongPress, isSelectionMode, isSelected }: CardProps) => (
...
export default function AveriaInspectionsListScreen() {
  ...
  const handlePress = (inspection: AveriaInspection) => {
    ...
    router.push({ pathname: '/averia-report-view' as any, params: { inspectionId: inspection.id } });
  };
  ...
  const handleDeleteSelected = () => {
    ...
    Alert.alert('Eliminar averías', `¿Eliminar ${count} avería${count > 1 ? 's' : ''}?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            setIsDeleting(true);
            for (const id of Array.from(selectedIds)) { await deleteAveriaInspection(id); }
            await loadInspections();
            cancelSelection();
            Alert.alert('Eliminadas', `${count} avería${count > 1 ? 's' : ''} eliminada${count > 1 ? 's' : ''}.`);
  ...
  const handleEditInspection = () => {
    ...
    router.push({ pathname: '/averia-machine-form' as any, params: { ...params, isEditing: 'true' } });
  };

  const handleDeleteSingle = () => {
    setMenuVisible(false);
    if (!selectedInspection) return;
    Alert.alert('Eliminar avería', `¿Eliminar la avería de "${selectedInspection.clientName}"?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Eliminar', style: 'destructive',
        onPress: async () => {
          try {
            ...
            await loadInspections();
            Alert.alert('Eliminada', 'Avería eliminada correctamente.');
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <View style={styles.container}>
        <LinearGradient colors={['#7c3aed', '#a78bfa', '#c4b5fd'] as any} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.header}>
          ...
          <Title style={styles.headerTitle}>Averías Registradas</Title>
        </LinearGradient>
        ...
        {loading ? (
          <View style={styles.emptyContainer}><Text>Cargando averías...</Text></View>
        ) : filteredInspections.length > 0 ? (
          <FlatList
            data={filteredInspections}
            renderItem={({ item }) => (
              <AveriaCard inspection={item} onPress={handlePress} onMenuPress={isSelectionMode ? undefined : handleMenuOpen} onLongPress={handleLongPress} isSelectionMode={isSelectionMode} isSelected={selectedIds.has(item.id)} />
            )}
          ...
          <View style={styles.emptyContainer}>
            <Text>No hay averías registradas.</Text>
            <Button mode="contained" onPress={() => router.push('/averia-machine-form' as any)} style={styles.newButton} icon="plus-circle" buttonColor="#7c3aed">Nueva Avería</Button>
          </View>
        )}
        ...
        <Menu visible={menuVisible} onDismiss={() => setMenuVisible(false)} anchor={menuPosition}>
          <Menu.Item onPress={() => { setMenuVisible(false); if (selectedInspection) router.push({ pathname: '/averia-report-view' as any, params: { inspectionId: selectedInspection.id } }); }} title="Ver informe" leadingIcon="file-document" />
          {userRole !== 'technician' && <Menu.Item onPress={handleEditInspection} title="Editar avería" leadingIcon="pencil" />}
