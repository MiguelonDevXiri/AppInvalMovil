...
const ensureLocalImageUri = async (uri: string): Promise<string> => {
  ...
  const targetPath = `${FileSystem.cacheDirectory || FileSystem.documentDirectory}instalaciones_img_${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
...
export const generateInstalacionesHTML = async (report: InstalacionInspection): Promise<string> => {
  try {
    ...
    const sitePhotos = await getImagesBase64Sequential(sitePhotoSources);
    ...
    const finalPhotos = await getImagesBase64Sequential(finalPhotoSources);
