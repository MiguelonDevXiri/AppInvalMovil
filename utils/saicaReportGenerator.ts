...
import type { SaicaInspection } from './saicaInspectionStorage';
import { renderSafetySummaryHTML } from './safetyChecklist';

// ==================== HELPERS ====================

const generateFileName = (report: SaicaInspection): string => {
  try {
    ...
    return `${cleanPlate}_${cleanClient}_SAICA_${cleanDate}.pdf`;
  } catch (error) {
    return `SAICA_${Date.now()}.pdf`;
  }
};
...
export const generateSaicaHTML = async (report: SaicaInspection): Promise<string> => {
  try {
    ...
    const photosSolucion = await processImagesParallel(report.solucionPhotos || [], WIDTH_EVIDENCE);

    // Procesar fotos de cada informe Saica
    const interventionsWithPhotos: { description: string; proposedSolution?: string; photos: string[] }[] = [];
    for (const intervention of (report.interventions || [])) {
      const photos = await processImagesParallel(intervention.photos || [], WIDTH_EVIDENCE);
      interventionsWithPhotos.push({
        ...intervention,
        photos,
      });
    }

    // No firmas en informes Saica
          ...
          ${photosGeneral.map((photo, i) => `
            <div class="photo-item">
              <div class="photo-section">
                <div class="photo-title">📸 ${i === 0 ? 'Foto general' : i === 1 ? 'Foto de placa' : `Foto ${i + 1}`}</div>
    ...
    // HTML informes Saica detectadas
    let interventionsHTML = '';
    if (interventionsWithPhotos.length > 0) {
      interventionsHTML = interventionsWithPhotos.map((d, i) => `
        <div class="section">
          <div class="section-header orange">🔧 INTERVENCIÓN NECESARIA ${i + 1}</div>
        ...
        ${d.photos.length > 0 ? `
          <div class="photos-grid">
            ${d.photos.map((photo, j) => `
              <div class="photo-item">
                <div class="photo-section">
                  <div class="photo-title">📸 Foto intervención ${i + 1} - ${j + 1}</div>
                  <div class="photo-container"><img src="${photo}" class="photo" /></div>
                </div>
              </div>
            `).join('')}
    ...
    return `
      ...
      <body>
        <div class="header">
          <div class="header-left">
            ...
            <div class="header-text">
              <h1>INFORME DE INTERVENCIÓN NECESARIA</h1>
        ...
        <div class="section">
          <div class="section-header orange">🛠️ PROPUESTA DE SOLUCIÓN</div>
    ...
    console.error('❌ Error en generateSaicaHTML:', error);
    throw error;
  }
};
...
export const shareSaicaPDFReport = async (report: SaicaInspection, onProgress?: (percent: number, text: string) => void, autoUploadOnly: boolean = false): Promise<boolean> => {
  try {
    onProgress?.(15, 'Procesando fotos...');
    const reportHTML = await generateSaicaHTML(report);
    ...
    if (Platform.OS === 'ios' || Platform.OS === 'android') {
      ...
      try {
        ...
        const recordId = sanitize(((report as { id?: string }).id || report.avisoDate || '').slice(0, 8));
        const storagePath = `saica/${sanitize(report.licensePlate)}_${sanitize(report.clientName)}_${sanitize(report.avisoDate)}_${recordId}/informe_${customFileName}`;
      ...
      await Sharing.shareAsync(finalUri, {
        ...
        dialogTitle: `Informe Informes Saica - ${report.clientName}`,
        UTI: 'com.adobe.pdf',
      });
...
export default { generateSaicaHTML, shareSaicaPDFReport };
