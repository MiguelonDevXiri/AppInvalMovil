...
const getImageBase64 = async (uri: string): Promise<string> => {
  return getImageBase64WithOrientation(uri, 500, false);
};
...
const getThumbnailBase64 = async (uri: string): Promise<string> => {
  return getImageBase64WithOrientation(uri, 700, true);
};
...
const getMachineTypeAbbreviation = (machineTypeId: string): string => {
...
const exportAllImages = async (
  ...
  try {
    ...
    // Exportar fotos generales
    if (generalPhotos && generalPhotos.photos) {
      console.log('Exportando fotos generales...');
      
      if (generalPhotos.photos.front) {
        imageCounter++;
        const path = await copyImageWithNewName(generalPhotos.photos.front, 'Frente', imageCounter);
        if (path) exportedPaths.push(path);
      }
      
      if (generalPhotos.photos.back) {
        imageCounter++;
        const path = await copyImageWithNewName(generalPhotos.photos.back, 'Posterior', imageCounter);
        if (path) exportedPaths.push(path);
      }
      
      if (generalPhotos.photos.left) {
        imageCounter++;
        const path = await copyImageWithNewName(generalPhotos.photos.left, 'Lateral_Izquierdo', imageCounter);
        if (path) exportedPaths.push(path);
      }
      
      if (generalPhotos.photos.right) {
        imageCounter++;
        const path = await copyImageWithNewName(generalPhotos.photos.right, 'Lateral_Derecho', imageCounter);
        if (path) exportedPaths.push(path);
      }
    }
...
const processComments = async (machine: Machine): Promise<string> => {
  ...
  if (machine.commentsWithPhotos && machine.commentsWithPhotos.length > 0) {
    for (let i = 0; i < machine.commentsWithPhotos.length; i++) {
      ...
      htmlContent += `
        <div style="margin-bottom: 20px; page-break-inside: avoid;">
          <strong style="color:#0056b3;display:block;margin-top:10px;margin-bottom:5px;font-size:14px;">
            Comentario #${i + 1}:
          </strong>
      ...
      if (comment.photoUri) {
        try {
          const photoBase64 = await getImageBase64(comment.photoUri);
          htmlContent += `
            <div style="margin:10px 0;text-align:center;">
              <img src="${photoBase64}" class="comment-photo" />
            </div>
          `;
        } catch (error) {
          console.error('Error al procesar foto del comentario:', error);
        }
      }
    ...
    htmlContent = htmlContent.replace(
      /#(\d+):/g, 
      '<strong style="color:#0056b3;display:block;margin-top:10px;margin-bottom:3px;font-size:14px;">Comentario #$1:</strong>'
    );
  }
...
export const generateHTMLReport = async (
  ...
  if (generalPhotos && generalPhotos.photos) {
    ...
    generalPhotosThumbnails = `
      <div class="header-photos">
        <div class="header-photos-title">Fotos Generales:</div>
        <div class="header-photos-grid">
  ...
  if (checklistResults && checklistResults.results) {
    ...
    checklistHtml += `<h2>Resultados del Checklist</h2>`;
    
    const machineChecklist = getChecklistByMachineType(machine.machineType || 'otros');
    
    for (const category of machineChecklist) {
      const hasItemsInCategory = category.items.some(item => 
      ...
      if (hasItemsInCategory) {
        ...
        for (const item of category.items) {
          if (checklistResults.results[item.id] !== undefined) {
            ...
            if (status === 'fail' && checklistResults.photos && checklistResults.photos[item.id]) {
              const itemPhotos = checklistResults.photos[item.id];
              
              let photoArray: any[] = [];
              
              if (Array.isArray(itemPhotos)) {
                photoArray = itemPhotos;
              } else if (typeof itemPhotos === 'string') {
                photoArray = [itemPhotos];
              } else if (typeof itemPhotos === 'object' && itemPhotos !== null) {
                photoArray = [itemPhotos];
              }
                ...
                checklistHtml += `
                  <tr>
                    <td colspan="2" class="photo-cell">
                      <div class="evidence-photos-grid">
                `;
                
                for (let i = 0; i < photoArray.length; i++) {
                  const { uri, comment } = processPhotoItem(photoArray[i]);
                  
                  try {
                    const base64Image = await getImageBase64(uri);
                    checklistHtml += `
                      <div class="evidence-photo-container">
                        <img src="${base64Image}" class="evidence-photo" 
                          onerror="this.onerror=null; this.src='${getPlaceholderImageBase64()}';" />
                        ${comment ? `<div class="photo-comment">${comment}</div>` : ''}
                      </div>
                    `;
                  } catch (error) {
                    console.error(`Error procesando imagen ${i+1} del ítem ${item.id}:`, error);
                    checklistHtml += `
                      <div class="evidence-photo-container">
                        <img src="${getPlaceholderImageBase64()}" class="evidence-photo" />
                        ${comment ? `<div class="photo-comment">${comment}</div>` : ''}
                        <p class="error-text">Error al cargar la imagen</p>
                      </div>
                    `;
                  }
                }
  ...
  return `
    ...
    <body>
      <div class="header">
        <div class="header-content">
          ...
          <div class="header-left">
            ...
            <div class="header-date">Fecha: ${date}</div>
          </div>
          ${generalPhotosThumbnails}
        </div>
      </div>
      ...
      <div class="machine-info">
        ...
        <p><strong>Marca:</strong> ${machine.brand || 'No especificado'}</p>
        <p><strong>Modelo:</strong> ${machine.model || 'No especificado'}</p>
        <p><strong>Nº Serie:</strong> ${machine.serialNumber || 'No especificado'}</p>
        <p><strong>Matrícula:</strong> ${machine.licensePlate || 'No especificado'}</p>
        <p><strong>Cliente:</strong> ${machine.clientName}</p>
        <p><strong>Ubicación:</strong> ${machine.location || 'No especificado'}</p>
        <p><strong>Revisado por:</strong> ${machine.reviewedBy || 'No especificado'}</p>
      </div>
      ...
      <div class="footer">
        <p>Informe generado por la aplicación de Taller de Máquinas</p>
