...
const ensureLocalImageUri = async (uri: string): Promise<string> => {
  ...
  const targetPath = `${FileSystem.cacheDirectory || FileSystem.documentDirectory}averias_img_${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
...
export const generateAveriasHTML = async (report: AveriaInspection): Promise<string> => {
  try {
    ...
    const generalResults = await Promise.all(generalSources.map(src => getImageBase64(src).catch(() => '')));
    const photosGeneral = generalResults.filter(img => img !== '');

    // Procesar fotos de solución
    const solucionResults = await Promise.all(
      (report.solucionPhotos || []).map(p => getImageBase64(p).catch(() => ''))
    );
    const photosSolucion = solucionResults.filter(img => img !== '');

    // Procesar fotos de cada avería
    const defectsWithPhotos: { description: string; photos: string[] }[] = [];
    for (const defect of (report.defects || [])) {
      const photoResults = await Promise.all(
        (defect.photos || []).map(p => getImageBase64(p).catch(() => ''))
      );
      defectsWithPhotos.push({
        description: defect.description,
        photos: photoResults.filter(img => img !== ''),
      });
    }
    ...
    if (report.materiales && report.materiales.length > 0) {
      const valid = report.materiales.filter(m => m.name || m.quantity);
      if (valid.length > 0) {
        materialesRows = valid.map(m => `
          <tr>
            <td>${m.name || '-'}</td>
    ...
    // HTML fotos generales
    let fotosGeneralesHTML = '';
    if (photosGeneral.length > 0) {
      fotosGeneralesHTML = `
        <div class="photos-grid">
          ${photosGeneral.map((photo, i) => `
            <div class="photo-item">
              <div class="photo-section">
                <div class="photo-title">📸 Foto General ${i + 1}</div>
    ...
    if (defectsWithPhotos.length > 0) {
      defectsHTML = defectsWithPhotos.map((d, i) => `
        ...
        ${d.photos.length > 0 ? `
          <div class="photos-grid">
            ${d.photos.map((photo, j) => `
              <div class="photo-item">
                <div class="photo-section">
                  <div class="photo-title">📸 Foto Avería ${i + 1} - ${j + 1}</div>
                  <div class="photo-container"><img src="${photo}" class="photo" /></div>
                </div>
              </div>
            `).join('')}
    ...
    // HTML fotos solución
    let fotosSolucionHTML = '';
    if (photosSolucion.length > 0) {
      fotosSolucionHTML = `
        <div class="photos-grid">
          ${photosSolucion.map((photo, i) => `
            <div class="photo-item">
              <div class="photo-section">
                <div class="photo-title">📸 Foto Solución ${i + 1}</div>
                <div class="photo-container"><img src="${photo}" class="photo" /></div>
              </div>
            </div>
          `).join('')}
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .signatures-section { margin-top: 30px; display: grid; grid-template-columns: 1fr 1fr; gap: 30px; page-break-inside: avoid; }
          .signature-box { text-align: center; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background: #f8fafc; }
          .signature-label { font-weight: bold; color: #0f2f57; margin-bottom: 15px; font-size: 11px; text-transform: uppercase; }
          .signature-image-container { min-height: 100px; max-height: 120px; display: flex; align-items: center; justify-content: center; background: #fff; border: 1.5px solid #e2e8f0; border-radius: 8px; margin-bottom: 15px; padding: 10px; overflow: hidden; }
          .signature-image { max-width: 100%; max-height: 100px; object-fit: contain; }
          .signature-placeholder { color: #d1d5db; font-style: italic; font-size: 11px; }
    ...
    console.error('❌ Error en generateAveriasHTML:', error);
    throw error;
  }
};
...
export const shareAveriasPDFReport = async (report: AveriaInspection, onProgress?: (percent: number, text: string) => void): Promise<boolean> => {
  try {
    onProgress?.(15, 'Procesando fotos...');
    const reportHTML = await generateAveriasHTML(report);
    ...
    if (Platform.OS === 'ios' || Platform.OS === 'android') {
      ...
      await Sharing.shareAsync(finalUri, {
        ...
        dialogTitle: `Informe Averías - ${report.clientName}`,
        UTI: 'com.adobe.pdf',
      });

      Alert.alert('Informe generado', `PDF: ${customFileName}`, [{ text: 'OK' }]);
      return true;
    } else {
...
export default { generateAveriasHTML, shareAveriasPDFReport };
