...
export const generateAveriasHTML = async (report: AveriaInspection): Promise<string> => {
  try {
    ...
    const photosSolucion = await getImagesBase64Sequential(report.solucionPhotos || []);

    // Procesar fotos de cada avería
    const defectsWithPhotos: { description: string; photos: string[] }[] = [];
    for (const defect of (report.defects || [])) {
      const photos = await getImagesBase64Sequential(defect.photos || []);
...
export const shareAveriasPDFReport = async (report: AveriaInspection, onProgress?: (percent: number, text: string) => void, autoUploadOnly: boolean = false): Promise<boolean> => {
  try {
    ...
    if (Platform.OS === 'ios' || Platform.OS === 'android') {
      ...
      try {
        ...
        const storagePath = `averias/${sanitize(report.licensePlate)}_${sanitize(report.clientName)}_${sanitize(report.avisoDate)}_${recordId}/informe_${customFileName}`;
